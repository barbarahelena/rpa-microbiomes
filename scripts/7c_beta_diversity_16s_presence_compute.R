...
##
## Companion to 7a_beta_diversity_16s_compute.R, which runs the same analysis
## on the two abundance-weighted metrics (Bray-Curtis, weighted UniFrac).
## Those are dominated by the handful of abundant taxa, so they are the
## metrics least sensitive to a difference concentrated in the rare tail -
## which is exactly what the alpha diversity results point at (migrant groups
## carry more taxa at lower evenness). Presence/absence metrics weight every
## observed taxon equally, so comparing ethnicity's R2 between the two
## families says whether the signal sits in the abundant core or the rare
## fraction. 7d_beta_diversity_16s_presence_report.R does that comparison.
##
...
## 7d_beta_diversity_16s_presence_report.R can rebuild every plot/table
## without repeating the permutation tests.
##
## The sample filtering below is deliberately identical to 7a's - 7d pairs
## this cache against 7a's, and the comparison is only valid if both cover
## the same samples.
...
## a real run. Example: BETA_DIV_TEST_N=40 Rscript scripts/7c_beta_diversity_16s_presence_compute.R
...
for (site_name in names(sites)) {
    ...
    ## Presence/absence counterparts of the two metrics 7a computes:
    ## unweighted UniFrac pairs with weighted UniFrac (phylogenetic),
    ## binary Jaccard pairs with Bray-Curtis (taxonomic).
    ##
    ## binary = TRUE is required for Jaccard. phyloseq::distance() forwards it
    ## to vegan::vegdist(), whose default is the *abundance* form of Jaccard -
    ## without it we would get a third abundance-weighted metric rather than a
    ## presence/absence one.
    distances <- list(
        "Unweighted UniFrac" = phyloseq::distance(ps, method = "unifrac"),
        "Jaccard"            = phyloseq::distance(ps, method = "jaccard", binary = TRUE)
    )
